![img](封面圖/工作坊海報V2.png)


## 關於我

我是一個有十年經驗的工程師, 曾服務賣牛肉麵的公司, 電風工廠

可以在我的 [codepen](https://codepen.io/weber87na) [blog](https://blog.lasai.com.tw/) 或 [YT](https://www.youtube.com/@ladisai) 找到我

![img](封面圖/辛普森鼠.jpg)

## 課程地圖

### 1. 課程總覽與基礎環境

| 主題 | 狀態 | 教學文件 |
| --- | --- | --- |
| 開發環境建置 | ✅ | [環境建置](環境建置.md) |
| 手機設定 codex | ✅ | [手機設定 codex](手機設定codex.md) |
| 常見資料格式 | ✅ | [資料格式](資料格式.md) |
| Terminal 基本命令 | ✅ | [基本命令](基本命令.md) |
| Prompt 與 AI 使用概念 | ✅ | [提示詞及 AI 使用概念](提示詞及AI使用概念.md) |

### 2. 資料、文件與辦公室應用

| 主題 | 狀態 | 教學文件 |
| --- | --- | --- |
| 資料夾與文件整理 | ✅ | [資料與文件夾整理](資料與文件夾整理.md) |
| 信件整理 | ✅ | [信件整理](信件整理.md) |
| 手寫辨識 | ✅ | [手寫辨識](手寫辨識.md) |
| Excel 與 CSV | ✅ | [Excel / CSV 操作](excel_csv_操作.md) |
| Word 資料填寫 | ✅ | [Word 資料填寫](word資料填寫.md) |
| 履歷與週報範例 | ✅ | [週報製作紀錄](週報.md) |
| Skill 範例與風格提煉 | ✅ | [Skill 範例](skill範例.md) |
| 錯字用詞修正 | ✅ | [錯字用詞修正](錯字用詞修正.md) |
| 網站截圖上傳 Google Drive | ✅ | [網站截圖上傳 Google Drive](網站截圖_上傳至_google_drive.md) |
| Computer Use 操作 LINE | ✅ | [Computer Use 操作 LINE](computer_use_操作_line.md) |
| 論文 PDF 文件翻譯 | ✅ | [論文 PDF 文件翻譯](論文_pdf_文件翻譯.md) |

### 3. AI 多媒體實戰

| 主題 | 狀態 | 教學文件 |
| --- | --- | --- |
| AI 圖片生成：文字生圖與圖片轉換 | ✅ | [AI 多媒體實戰](AI多媒實戰.md) |
| 副歌擷取聽海 | ✅ | [副歌擷取聽海](聽海.md) |
| 影片字幕與會議重點摘要 | ✅ | [影片字幕與會議重點摘要](產生影片字幕.md) |
| ChatGPT Work and Gemini Spark 示範 | ✅ | [ChatGPT Work and Gemini Spark 示範](網頁work示範.md) |
| 國內旅遊規劃 | ✅ | [旅遊規劃](旅遊規劃.md) |
| Remotion 製作影片 | ✅ | [Remotion 製作影片](remotion製作影片.md) |
| Blender 簡單建模 | ✅ | [Blender 建模](blender建模.md) |

### 4. Vibe Coding 與前端實作

| 主題 | 狀態 | 教學文件 |
| --- | --- | --- |
| Vibe Coding 前端 QR Code、ToDo List、擲筊與佛珠、照妖鏡 | ✅ | [前端 Vibe Coding 範例](前端vibecoding範例.md) |

### 5. 後端與 API

| 主題 | 狀態 | 教學文件 |
| --- | --- | --- |
| Vibe Coding 後端 ToDo List & 將網站變成 Skill | ✅ | [後端 Vibe Coding 範例](後端todo.md) |
---